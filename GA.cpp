...
uniform_real_distribution<> dis(0.0, 1.0); // Uniform distribution for random numbers 
between 0 and 1
...
// Function to calculate the cost (fitness) J(x) = ||y - Hx||
double costFunction(const vector<double>& y, const vector<vector<double>>& H, const 
vector<double>& x) {
 double error = 0.0;
...
vector<double> tournamentSelection(const vector<vector<double>>& population, const 
vector<double>& fitness, const vector<double>& y, const vector<vector<double>>& H) {
...
// Genetic Algorithm
vector<double> geneticAlgorithm(const vector<double>& y, const 
vector<vector<double>>& H) {
 ...
 if (gen % 100 == 0) {
 cout << "Generation " << gen << ", Best Cost: " << *min_element(fitness.begin(), 
fitness.end()) << endl;
 }
 }
